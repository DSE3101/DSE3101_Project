...
boldText <-function(text){
  tags$p(text, class = "boldText")
}

head1 <- function(text){
  tags$p(text, class ="head1")
}

head2 <- function(text){
  tags$p(text, class = "head2")
}
```
...
ui <- navbarPage(
  ...
  tabPanel("Machine Learning Model",
           mainPanel(
             boldText("Test!"),
             bodyText("Content for Real Time vs Vintage Data.")
           )
  ),
  
  tabPanel("Time Series Model",
           mainPanel(
             bodyText("Content for another tab.")
           )
  )
)
```
