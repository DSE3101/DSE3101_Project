...
def AR_MODEL(year_input, quarter_input):
    ...
    for h_step in range(8):
        ...
        for k in range(1, k_max + 1):
            ...
            X = real_time_y.iloc[:-(8+h_step), 1:k+1].values  # Select lagged columns as features
            y = real_time_y['yt'].values[:-(8+h_step)]  # Select the original column as target
            # Perform LOOCV
            loocv_mse = 0
        ...
        linear_model.fit(real_time_y.iloc[:-(8+h_step), 1:optimal_lag+1], real_time_y.iloc[:-(8+h_step), 0])
        ...
        # backtest_X_train = real_time_y.iloc[:(-8-8-h_step), 1:optimal_lag+1]
        # backtest_y_train = real_time_y.iloc[:(-8-8-h_step), 0]
        ...
        # pseudo_out_of_sample_X = real_time_y.iloc[(-8-8-h_step):(-8-h_step), 1:optimal_lag+1]
        ...
        # backtest_actual = real_time_y.iloc[(-8-8-h_step):(-8-h_step), 0].to_list()
        # print(backtest_actual)
